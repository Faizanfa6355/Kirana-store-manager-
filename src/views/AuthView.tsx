...
export const AuthView: React.FC = () => {
  ...
  const [mode, setMode] = useState<'login' | 'signup' | 'forgot' | 'admin'>('login');
  ...
  if (mode === 'admin') {
    return <AdminLoginView onBackToNormalLogin={() => setMode('login')} />;
  }
        ...
        <div className="pt-2 text-center text-xs text-slate-500 dark:text-slate-400 space-y-2">
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors cursor-pointer"
            >
